...
const headerCell = (text, width) => {
  return new TableCell({
    width: { size: width, type: WidthType.DXA },
    shading: { color: HEADER_COLOR, type: ShadingType.CLEAR },
    verticalAlign: VerticalAlign.CENTER,
    borders: {
...
const fullWidthTable = (headers, rows, colWidths) => {
  const headerRow = new TableRow({
    children: headers.map((h, i) => headerCell(h, colWidths[i])),
  });

  const dataRows = rows.map((row, rowIdx) => {
    const bgColor = rowIdx % 2 === 0 ? ALT_ROW_COLOR : 'FFFFFF';
    return new TableRow({
      children: row.map((c, colIdx) => cell(c, colWidths[colIdx], bgColor)),
    });
  });
...
// Heading paragraph
const heading = (text, level, color = HEADER_COLOR) => {
  return new Paragraph({
    heading: level,
    spacing: { before: 200, after: 200 },
    children: [new TextRun({ text, bold: true, color, size: level === HeadingLevel.HEADING_1 ? 52 : 44 })],
  });
};
...
const deploymentRunbook = () => {
  ...
  s.push(
    new Paragraph({
      alignment: AlignmentType.CENTER,
      spacing: { after: 100 },
      children: [new TextRun({ text: 'SYNTONIQA v1.0', bold: true, color: HEADER_COLOR, size: 52 })],
    }),
    new Paragraph({
      alignment: AlignmentType.CENTER,
      spacing: { after: 300 },
      children: [new TextRun({ text: 'Deployment Runbook', bold: true, color: ACCENT_COLOR, size: 44 })],
    }),
    new Paragraph({
      alignment: AlignmentType.CENTER,
      spacing: { after: 600 },
      children: [new TextRun({ text: 'Field Service Management Platform for MRS Lely Center', italics: true })],
    }),
    new Paragraph({
      alignment: AlignmentType.CENTER,
      spacing: { after: 400 },
  ...
  s.push(
    ...
    new Paragraph({ text: '2. Architecture Overview' }),
    new Paragraph({ text: '3. Prerequisites & Accounts' }),
    new Paragraph({ text: '4. Environment Variables Reference' }),
    new Paragraph({ text: '5. New Tenant Setup (Step-by-Step)' }),
    new Paragraph({ text: '6. Deployment Procedures' }),
    new Paragraph({ text: '7. CI/CD Pipeline & GitHub Actions' }),
    new Paragraph({ text: '8. Monitoring & Alerting' }),
    new Paragraph({ text: '9. Backup & Restore Procedures' }),
    new Paragraph({ text: '10. Health Checks & Validation' }),
    new Paragraph({ text: '11. Rollback Procedures' }),
    new Paragraph({ text: '12. Troubleshooting Guide' }),
    new PageBreak()
  );
  ...
  s.push(
    ...
    new Paragraph({
      ...
      spacing: { after: 100 },
      children: [new TextRun({ text: 'Key Deployment Facts:', bold: true })],
    }),
    ...
    new Paragraph({
      ...
      spacing: { after: 400 },
      children: [new TextRun({ text: 'This document is for Platform Engineers, DevOps, and System Administrators.', italics: true })],
    }),
    new PageBreak()
  );
  ...
  s.push(
    heading('2. ARCHITECTURE OVERVIEW', HeadingLevel.HEADING_2),
    new Paragraph({ text: 'Syntoniqa follows a serverless, edge-first architecture:', spacing: { after: 200 } }),
    fullWidthTable(
    ...
    new Paragraph({
      text: 'Data Flow:',
      spacing: { after: 100 },
      children: [new TextRun({ text: 'Data Flow:', bold: true })],
    }),
    new Paragraph({
  ...
  // 3. Prerequisites
  s.push(
    heading('3. PREREQUISITES & ACCOUNTS', HeadingLevel.HEADING_2),
    new Paragraph({ text: 'Before deploying Syntoniqa, ensure you have active accounts and credentials for:', spacing: { after: 200 } }),
    fullWidthTable(
  ...
  s.push(
    heading('4. ENVIRONMENT VARIABLES REFERENCE', HeadingLevel.HEADING_2),
    new Paragraph({
    ...
    new Paragraph({
      ...
      spacing: { after: 100 },
      children: [new TextRun({ text: 'Local Development (.env file):', bold: true })],
    }),
    new Paragraph({
      text: 'export CLOUDFLARE_API_TOKEN=xxxxxxx\nexport SUPABASE_URL=https://sajzbanhkehkkhhgztkq.supabase.co\nexport SUPABASE_SERVICE_KEY=xxxxxxx\nexport SQ_TOKEN=xxxxxxx\nexport GEMINI_KEY=xxxxxxx\nexport TELEGRAM_BOT_TOKEN=xxxxxxx\nexport RESEND_API_KEY=xxxxxxx\nexport TENANT_ID=785d94d0-b947-4a00-9c4e-3b67833e7045',
      spacing: { after: 200, line: 240 },
      children: [new TextRun({ text: 'export CLOUDFLARE_API_TOKEN=xxxxxxx\n... (all 8 vars)', color: '666666', size: 20 })],
    }),
    new Paragraph({
  ...
  // 5. New Tenant Setup
  s.push(
    heading('5. NEW TENANT SETUP (STEP-BY-STEP)', HeadingLevel.HEADING_2),
    new Paragraph({ text: 'Follow these steps to onboard a new tenant:', spacing: { after: 200 } }),

    new Paragraph({
      text: 'STEP 1: Create Supabase Project',
      spacing: { before: 100, after: 80 },
      children: [new TextRun({ text: 'STEP 1: Create Supabase Project', bold: true, color: ACCENT_COLOR })],
    }),
    new Paragraph({
    ...
    new Paragraph({
      ...
      spacing: { before: 100, after: 80 },
      children: [new TextRun({ text: 'STEP 2: Initialize Database Schema', bold: true, color: ACCENT_COLOR })],
    }),
    new Paragraph({
    ...
    new Paragraph({
      ...
      spacing: { before: 100, after: 80 },
      children: [new TextRun({ text: 'STEP 3: Configure Cloudflare Worker', bold: true, color: ACCENT_COLOR })],
    }),
    new Paragraph({
    ...
    new Paragraph({
      ...
      spacing: { before: 100, after: 80 },
      children: [new TextRun({ text: 'STEP 4: Prepare GitHub Repo', bold: true, color: ACCENT_COLOR })],
    }),
    new Paragraph({
    ...
    new Paragraph({
      ...
      spacing: { before: 100, after: 80 },
      children: [new TextRun({ text: 'STEP 5: Configure Telegram Bot (Optional)', bold: true, color: ACCENT_COLOR })],
    }),
    new Paragraph({
    ...
    new Paragraph({
      ...
      spacing: { before: 100, after: 80 },
      children: [new TextRun({ text: 'STEP 6: Add Initial Admin User', bold: true, color: ACCENT_COLOR })],
    }),
    new Paragraph({
      text: '(1) Supabase SQL Editor: INSERT INTO utenti (id, nome, ruolo, password_hash) ...\n(2) OR use Syntoniqa admin dashboard → Users → Create Admin\n(3) Store credentials securely (1Password)',
      spacing: { after: 200, line: 240 },
    }),
    ...
    new Paragraph({
      ...
      spacing: { before: 100, after: 80 },
      children: [new TextRun({ text: 'STEP 7: Validation & Health Check', bold: true, color: ACCENT_COLOR })],
    }),
    new Paragraph({
  ...
  // 6. Deployment
  s.push(
    heading('6. DEPLOYMENT PROCEDURES', HeadingLevel.HEADING_2),
    new Paragraph({
      text: 'Deploy Backend (Cloudflare Worker):',
      spacing: { after: 100 },
      children: [new TextRun({ text: 'Deploy Backend (Cloudflare Worker):', bold: true, color: ACCENT_COLOR })],
    }),
    new Paragraph({
      text: 'export CLOUDFLARE_API_TOKEN=<your_cf_token>\nnpm install -g wrangler\ncd syntoniqa\nnpm install\nwrangler deploy',
      spacing: { after: 200, line: 240 },
      children: [new TextRun({ text: 'export CLOUDFLARE_API_TOKEN=<your_cf_token>\nwrangler deploy', color: '666666', size: 20 })],
    }),

    new Paragraph({
      text: 'Deploy Frontend (GitHub Pages):',
      spacing: { after: 100 },
      children: [new TextRun({ text: 'Deploy Frontend (GitHub Pages):', bold: true, color: ACCENT_COLOR })],
    }),
    new Paragraph({
      text: 'git add admin_v1.html index_v2.html white_label_config.js\ngit commit -m "chore: v1.0 release"\ngit push origin main',
      spacing: { after: 200, line: 240 },
      children: [new TextRun({ text: 'git add admin_v1.html index_v2.html\ngit push origin main', color: '666666', size: 20 })],
    }),
  ...
  // 7. CI/CD
  s.push(
    heading('7. CI/CD PIPELINE & GITHUB ACTIONS', HeadingLevel.HEADING_2),
    new Paragraph({
    ...
    new Paragraph({
      ...
      spacing: { after: 200, line: 240 },
      children: [new TextRun({ text: 'name: Deploy Syntoniqa\non:\n  push:\n    branches: [main]\njobs:\n  deploy: ...', color: '666666', size: 18 })],
    }),

    new Paragraph({
      text: 'Setup GitHub Secrets (repo Settings → Secrets & Variables):',
      spacing: { before: 100, after: 100 },
      children: [new TextRun({ text: 'Setup GitHub Secrets (repo Settings → Secrets & Variables):', bold: true })],
    }),
    fullWidthTable(
    ...
    new Paragraph({
      ...
      spacing: { before: 100, after: 100 },
      children: [new TextRun({ text: 'Branch Strategy:', bold: true })],
    }),
  ...
  // 8. Monitoring
  s.push(
    heading('8. MONITORING & ALERTING', HeadingLevel.HEADING_2),
    new Paragraph({
      text: 'Cloudflare Worker Monitoring:',
      spacing: { after: 100 },
      children: [new TextRun({ text: 'Cloudflare Worker Monitoring:', bold: true, color: ACCENT_COLOR })],
    }),
    new Paragraph({
    ...
    new Paragraph({
      ...
      spacing: { before: 100, after: 100 },
      children: [new TextRun({ text: 'Supabase Monitoring:', bold: true, color: ACCENT_COLOR })],
    }),
    new Paragraph({
    ...
    new Paragraph({
      ...
      spacing: { before: 100, after: 100 },
      children: [new TextRun({ text: 'Recommended: Third-party APM Integration', bold: true })],
    }),
    new Paragraph({
  ...
  // 9. Backup & Restore
  s.push(
    heading('9. BACKUP & RESTORE PROCEDURES', HeadingLevel.HEADING_2),
    new Paragraph({
      text: 'Supabase Backup (Automatic):',
      spacing: { after: 100 },
      children: [new TextRun({ text: 'Supabase Backup (Automatic):', bold: true, color: ACCENT_COLOR })],
    }),
    new Paragraph({
    ...
    new Paragraph({
      ...
      spacing: { before: 100, after: 100 },
      children: [new TextRun({ text: 'Manual Backup (Recommended Weekly):', bold: true, color: ACCENT_COLOR })],
    }),
    new Paragraph({
      ...
      children: [
        new TextRun({
          text: 'pg_dump -h ... > backup_$(date +%Y%m%d_%H%M%S).sql',
          color: '666666',
          size: 20,
        }),
      ],
    ...
    new Paragraph({
      ...
      spacing: { before: 100, after: 100 },
      children: [new TextRun({ text: 'Restore from Backup:', bold: true, color: ACCENT_COLOR })],
    }),
    new Paragraph({
      text: 'psql -h sajzbanhkehkkhhgztkq.supabase.co -U postgres -d postgres < backup_YYYYMMDD_HHMMSS.sql',
      spacing: { after: 400, line: 240 },
      children: [
        new TextRun({ text: 'psql -h ... < backup_YYYYMMDD_HHMMSS.sql', color: '666666', size: 20 }),
      ],
    }),
    new PageBreak()
  );
  ...
  // 10. Health Checks
  s.push(
    heading('10. HEALTH CHECKS & VALIDATION', HeadingLevel.HEADING_2),
    new Paragraph({
      text: 'Post-Deployment Checklist:',
      spacing: { after: 100 },
      children: [new TextRun({ text: 'Post-Deployment Checklist:', bold: true, color: ACCENT_COLOR })],
    }),
    ...
    new Paragraph({
      text: 'Load Testing:',
      spacing: { before: 100, after: 100 },
      children: [new TextRun({ text: 'Load Testing:', bold: true, color: ACCENT_COLOR })],
    }),
    new Paragraph({
      text: 'ab -n 1000 -c 10 "https://your-worker.workers.dev?action=getAll&token=xyz"\nAcceptable: <100ms p99 latency, 0 errors',
      spacing: { after: 200, line: 240 },
      children: [
        new TextRun({ text: 'ab -n 1000 -c 10 ...\nAcceptable: <100ms p99 latency, 0 errors', color: '666666', size: 20 }),
      ],
    }),
    ...
    new Paragraph({
      ...
      spacing: { before: 100, after: 100 },
      children: [new TextRun({ text: 'Data Integrity Checks:', bold: true, color: ACCENT_COLOR })],
    }),
    new Paragraph({
      text: 'SELECT COUNT(*) FROM utenti WHERE tenant_id = ? AND deleted_at IS NULL;\nTrend: Monitor for unexpected spikes or drops',
      spacing: { after: 400, line: 240 },
      children: [
        new TextRun({ text: 'SELECT COUNT(*) FROM utenti ...\nTrend: Monitor for unexpected spikes or drops', color: '666666', size: 20 }),
      ],
    }),
    new PageBreak()
  );
  ...
  // 11. Rollback
  s.push(
    heading('11. ROLLBACK PROCEDURES', HeadingLevel.HEADING_2),
    new Paragraph({
      text: 'If Deployment Fails:',
      spacing: { after: 100 },
      children: [new TextRun({ text: 'If Deployment Fails:', bold: true, color: ACCENT_COLOR })],
    }),
    new Paragraph({
    ...
    new Paragraph({
      ...
      spacing: { before: 100, after: 100 },
      children: [new TextRun({ text: 'Database Rollback:', bold: true, color: ACCENT_COLOR })],
    }),
    new Paragraph({
  ...
  // 12. Troubleshooting
  s.push(
    heading('12. TROUBLESHOOTING GUIDE', HeadingLevel.HEADING_2),
    fullWidthTable(
    ...
    new Paragraph({
      text: 'Debug Commands:',
      spacing: { before: 100, after: 100 },
      children: [new TextRun({ text: 'Debug Commands:', bold: true })],
    }),
    new Paragraph({
    ...
    new Paragraph({
      text: `Syntoniqa v1.0 | Generated: ${new Date().toLocaleDateString()} | For questions: engineering@mrs-lely.it`,
      alignment: AlignmentType.CENTER,
      spacing: { after: 100 },
      children: [
        new TextRun({
  ...
  return new Document({ sections: s });
};
...
const uatReport = () => {
  ...
  s.push(
    ...
    new Paragraph({
      alignment: AlignmentType.CENTER,
      spacing: { after: 300 },
      children: [new TextRun({ text: 'USER ACCEPTANCE TEST (UAT) REPORT', bold: true, color: ACCENT_COLOR, size: 44 })],
    }),
    new Paragraph({
      alignment: AlignmentType.CENTER,
      spacing: { after: 600 },
  ...
  s.push(
    ...
    new Paragraph({
      ...
      spacing: { after: 100 },
      children: [new TextRun({ text: 'Test Results Summary:', bold: true })],
    }),
    fullWidthTable(
  ...
  s.push(
    heading('TEST COVERAGE BY FUNCTIONAL AREA', HeadingLevel.HEADING_2),
    fullWidthTable(
  ...
  // Detailed Results
  s.push(
    heading('DETAILED TEST RESULTS (43 SUITES)', HeadingLevel.HEADING_2),

    new Paragraph({
      text: 'Authentication & Authorization (3 suites, 18 tests)',
      spacing: { before: 100, after: 80 },
      children: [new TextRun({ text: 'Authentication & Authorization (3 suites, 18 tests)', bold: true, color: ACCENT_COLOR })],
    }),
    new Paragraph({
  ...
  // Performance Metrics
  s.push(
    heading('PERFORMANCE METRICS', HeadingLevel.HEADING_2),

    new Paragraph({
      text: 'API Response Times (Cloudflare Worker):',
      spacing: { after: 100 },
      children: [new TextRun({ text: 'API Response Times (Cloudflare Worker):', bold: true, color: ACCENT_COLOR })],
    }),
    fullWidthTable(
    ...
    new Paragraph({
      ...
      spacing: { before: 100, after: 100 },
      children: [new TextRun({ text: 'Database Query Performance (Supabase):', bold: true, color: ACCENT_COLOR })],
    }),
    new Paragraph({
  ...
  // Security
  s.push(
    heading('DATA INTEGRITY & SECURITY VALIDATION', HeadingLevel.HEADING_2),
    ...
    new Paragraph({
      ...
      spacing: { before: 100, after: 100 },
      children: [new TextRun({ text: 'Security Testing (OWASP Top 10):', bold: true, color: ACCENT_COLOR })],
    }),
    fullWidthTable(
    ...
    new Paragraph({
      ...
      spacing: { before: 100, after: 100 },
      children: [new TextRun({ text: 'Cross-Tenant Isolation:', bold: true, color: ACCENT_COLOR })],
    }),
    new Paragraph({
  ...
  s.push(
    heading('CROSS-BROWSER & MOBILE TESTING', HeadingLevel.HEADING_2),

    new Paragraph({
      text: 'Browser Compatibility:',
      spacing: { after: 100 },
      children: [new TextRun({ text: 'Browser Compatibility:', bold: true, color: ACCENT_COLOR })],
    }),
    fullWidthTable(
  ...
  s.push(
    heading('TELEGRAM BOT & INTEGRATION TESTING', HeadingLevel.HEADING_2),

    new Paragraph({
      text: 'Telegram Bot Commands:',
      spacing: { after: 100 },
      children: [new TextRun({ text: 'Telegram Bot Commands:', bold: true, color: ACCENT_COLOR })],
    }),
    new Paragraph({
    ...
    new Paragraph({
      ...
      spacing: { before: 100, after: 100 },
      children: [new TextRun({ text: 'Webhook Handling:', bold: true, color: ACCENT_COLOR })],
    }),
    new Paragraph({
  ...
  // Recommendations
  s.push(
    heading('RECOMMENDATIONS & NEXT STEPS', HeadingLevel.HEADING_2),

    new Paragraph({
      text: 'Pre-Production Recommendations:',
      spacing: { after: 100 },
      children: [new TextRun({ text: 'Pre-Production Recommendations:', bold: true, color: ACCENT_COLOR })],
    }),
    new Paragraph({
    ...
    new Paragraph({
      ...
      spacing: { before: 100, after: 100 },
      children: [new TextRun({ text: 'Known Limitations (Non-Blocking):', bold: true, color: ACCENT_COLOR })],
    }),
    new Paragraph({
    ...
    new Paragraph({
      ...
      spacing: { before: 100, after: 100 },
      children: [new TextRun({ text: 'Post-Launch Monitoring (First 30 Days):', bold: true, color: ACCENT_COLOR })],
    }),
    new Paragraph({
  ...
  // Sign-Off
  s.push(
    heading('UAT APPROVAL & SIGN-OFF', HeadingLevel.HEADING_2),

    new Paragraph({
      text: 'Test Summary:',
      spacing: { after: 100 },
      children: [new TextRun({ text: 'Test Summary:', bold: true })],
    }),
...
(async () => {
  try {
    ...
    console.log('   ✓ Saved: 07_Deployment_Runbook.docx\n');
    ...
    console.log('   ✓ Saved: 08_UAT_Report.docx\n');
